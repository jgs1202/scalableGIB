...
def edgeLength(data):
    ...
    for (index1, index2) in links:
        sx1, sy1 = nodes[index1]
        tx1, ty1 = nodes[index2]
        list.append(math.sqrt((sx1 - tx1) ** 2 + (sy1 - ty1) ** 2))
    return list
    ...
    for path in pathes:
        ...
        for file in os.listdir(path):
            if file != '.DS_Store':
                ...
                list.extend([type, data['groupSize'], data['pgroup'], data['pout'], crossing, aspect(data), spaceWasted(data), modularity(data), len(data['links']) , edgeLength(data)])
